...
brands = {"Celotex": "Scrapper_Celotex.py", "Recticel": "Scrapper_Recticel.py", 
          "Ecotherm": "Scrapper_Ecotherm.py", "Unilin": "Scrapper_Unilin.py", 
          "IKO": "Scrapper_IKO.py", "Mannok": "Scrapper_Mannok.py", 
          "Core-Products": "Scrapper_Core_Products.py", "Cladco": "Scrapper_Cladco.py", 
          "Novia": "Scrapper_Novia.py", "Powerlon": "Scrapper_Powerlon.py", 
          "Superfoil": "Scrapper_Superfoil.py"}
st.session_state.selected_brand = st.sidebar.selectbox("Select Brand", list(brands.keys()))
...
if st.session_state.selected_brand:
    ...
    if st.session_state.selected_date and os.path.exists(data_path):
        ...
        if st.sidebar.button("Run Script"):
            ...
            st.write("We are fetching data...It may take 40-50 minutes...")
            script_path = os.path.join(os.getcwd(), brands[st.session_state.selected_brand])
            with st.spinner("Running script..."):
                df = run_script(script_path)
                if df is not None:
                    st.sidebar.success("✅ Script executed and data updated!")
                    st.cache_data.clear()
                    st.rerun()  # Force reload to reflect updated data
                else:
                    st.sidebar.error("❌ Failed to run the script or update data.")
