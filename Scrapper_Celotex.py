...
def scrape_insulationonline(url):
    try:
        ...
        # Define product keys and their divisors
        product_divisors = {
            "xr4110": 10,
            "xr4130": 9,
            "pl4025": 26,
        ...
        for key, divisor in product_divisors.items():
            if key in url_lower:
                adjusted_price = round(price_value / divisor, 2)
...
def scrape_diybuildingsupplies(url):
    try:
        ...
        # Remove pound symbol and convert to float once
        price_value = float(price_current.replace("£", ""))

        if "thermaclass" in url.lower():
            price_value = round(price_value * 16, 2)
            return f"£{price_value}"
...
df = pd.read_excel(r"C:\Users\Priyanka\Documents\Search_for_product_on_Competitor's\Celotex & Recticel Links.xlsx", sheet_name="Celotex")
...
output_file_name = f"C:\\Users\\Priyanka\\Documents\\Price_Comp_Dashboard\\Compititor's_Price\\Celotex_Prices\\Celotex_Prices_{current_date}.xlsx"
